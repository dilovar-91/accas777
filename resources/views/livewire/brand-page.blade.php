            ...
            <div class="mx-auto max-w-screen-2xl px-4 md:px-8 overflow-y-auto "  style="height: 32vh;">
                <div class="grid grid-cols-7 gap-2 ">
                    @foreach ($brands as $mark)
                        <div wire:key="mark-id-{{ $mark->id }}" wire:click="loadData({{ $mark->id }})" class=" cursor-pointer flex h-12 items-center justify-center rounded-lg p-3 sm:h-11 text-white" style="background-color: @if($brand_id === $mark->id) #dc2626    @else #10b981  @endif;">
                            {{ $mark->name }}
                        </div>
                    @endforeach
                </div>
